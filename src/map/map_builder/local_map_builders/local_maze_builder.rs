...
impl MetaLocalMapBuilder for MazeBuilder {
    #[allow(dead_code)]
...
impl MazeBuilder {
    ...
    fn build(&mut self, rng : &mut RandomNumberGenerator, build_data : &mut LocalMapBuilder) {
        // Maze gen
        let mut maze = Grid::new((LOCAL_MAP_WIDTH / 2)-2, (LOCAL_MAP_HEIGHT / 2)-2, rng);
...
impl<'a> Grid<'a> {
    ...
    fn generate_maze(&mut self, build_data : &mut LocalMapBuilder) {
        let mut i = 0;
        loop {
    ...
    fn copy_to_map(&self, map : &mut LocalMap) {
        // Clear the map
        for i in map.tiles.iter_mut() { *i = LocalTileType::Wall; }

        for cell in self.cells.iter() {
            let x = cell.column + 1;
            let y = cell.row + 1;
            let idx = local_map_idx(x * 2, y * 2);

            map.tiles[idx] = LocalTileType::Floor;
            if !cell.walls[TOP] { map.tiles[idx - LOCAL_MAP_WIDTH as usize] = LocalTileType::Floor }
            if !cell.walls[RIGHT] { map.tiles[idx + 1] = LocalTileType::Floor }
            if !cell.walls[BOTTOM] { map.tiles[idx + LOCAL_MAP_WIDTH as usize] = LocalTileType::Floor }
            if !cell.walls[LEFT] { map.tiles[idx - 1] = LocalTileType::Floor }
        }
    }
}