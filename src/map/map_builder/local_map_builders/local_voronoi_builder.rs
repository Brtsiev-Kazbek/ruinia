...
impl MetaLocalMapBuilder for VoronoiCellBuilder {
    #[allow(dead_code)]
...
impl VoronoiCellBuilder {
    ...
    fn build(&mut self, rng : &mut RandomNumberGenerator, build_data : &mut LocalMapBuilder) {
        ...
        while voronoi_seeds.len() < self.n_seeds {
            let vx = rng.roll_dice(1, LOCAL_MAP_WIDTH-1);
            let vy = rng.roll_dice(1, LOCAL_MAP_HEIGHT-1);
            let vidx = local_map_idx(vx, vy);
        ...
        let mut voronoi_membership : Vec<i32> = vec![0 ; LOCAL_MAP_WIDTH as usize * LOCAL_MAP_HEIGHT as usize];
        for (i, vid) in voronoi_membership.iter_mut().enumerate() {
            let x = i as i32 % LOCAL_MAP_WIDTH;
            let y = i as i32 / LOCAL_MAP_WIDTH;
        ...
        for y in 1..LOCAL_MAP_HEIGHT-1 {
            for x in 1..LOCAL_MAP_WIDTH-1 {
                let mut neighbors = 0;
                let my_idx = local_map_idx(x, y);
                let my_seed = voronoi_membership[my_idx];
                if voronoi_membership[local_map_idx(x-1, y)] != my_seed { neighbors += 1; }
                if voronoi_membership[local_map_idx(x+1, y)] != my_seed { neighbors += 1; }
                if voronoi_membership[local_map_idx(x, y-1)] != my_seed { neighbors += 1; }
                if voronoi_membership[local_map_idx(x, y+1)] != my_seed { neighbors += 1; }

                if neighbors < 2 {
                    build_data.map.tiles[my_idx] = LocalTileType::Floor;
                }
            }
