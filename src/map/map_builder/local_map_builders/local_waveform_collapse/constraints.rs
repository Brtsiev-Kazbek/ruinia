...
pub fn build_patterns(map : &LocalMap, chunk_size: i32, include_flipping: bool, dedupe: bool) -> Vec<Vec<LocalTileType>> {
    let chunks_x = LOCAL_MAP_WIDTH / chunk_size;
    let chunks_y = LOCAL_MAP_HEIGHT / chunk_size;
    let mut patterns = Vec::new();

    for cy in 0..chunks_y {
        for cx in 0..chunks_x {
            // Normal orientation
            let mut pattern : Vec<LocalTileType> = Vec::new();
            ...
            if include_flipping {
                ...
                for y in start_y .. end_y {
                    for x in start_x .. end_x {
                        let idx = local_map_idx(end_x - (x+1), y);
                        pattern.push(map.tiles[idx]);
                    }
                }
                patterns.push(pattern);
                ...
                for y in start_y .. end_y {
                    for x in start_x .. end_x {
                        let idx = local_map_idx(x, end_y - (y+1));
                        pattern.push(map.tiles[idx]);
                    }
                }
                patterns.push(pattern);
                ...
                for y in start_y .. end_y {
                    for x in start_x .. end_x {
                        let idx = local_map_idx(end_x - (x+1), end_y - (y+1));
                        pattern.push(map.tiles[idx]);
                    }
                }
                patterns.push(pattern);
    ...
    if dedupe {
        ...
        let set: HashSet<Vec<LocalTileType>> = patterns.drain(..).collect(); // dedup
...
pub fn render_pattern_to_map(map : &mut LocalMap, chunk: &LocalMapChunk, chunk_size: i32, start_x : i32, start_y: i32) {
    let mut i = 0usize;
    for tile_y in 0..chunk_size {
        for tile_x in 0..chunk_size {
            let map_idx = local_map_idx(start_x + tile_x, start_y + tile_y);
    ...
    for (x,northbound) in chunk.exits[0].iter().enumerate() {
        if *northbound {
            let map_idx = local_map_idx(start_x + x as i32, start_y);
            map.tiles[map_idx] = LocalTileType::DownStairs;
        }
    }
    for (x,southbound) in chunk.exits[1].iter().enumerate() {
        if *southbound {
            let map_idx = local_map_idx(start_x + x as i32, start_y + chunk_size -1);
            map.tiles[map_idx] = LocalTileType::DownStairs;
        }
    }
    for (x,westbound) in chunk.exits[2].iter().enumerate() {
        if *westbound {
            let map_idx = local_map_idx(start_x, start_y + x as i32);
            map.tiles[map_idx] = LocalTileType::DownStairs;
        }
    }
    for (x,eastbound) in chunk.exits[2].iter().enumerate() {
        if *eastbound {
            let map_idx = local_map_idx(start_x + chunk_size - 1, start_y + x as i32);
            map.tiles[map_idx] = LocalTileType::DownStairs;
        }
    }
}

pub fn patterns_to_constraints(patterns: Vec<Vec<LocalTileType>>, chunk_size : i32) -> Vec<LocalMapChunk> {
    // Move into the new constraints object
    let mut constraints : Vec<LocalMapChunk> = Vec::new();
    for p in patterns {
        let mut new_chunk = LocalMapChunk{
            pattern: p,
        ...
        for x in 0..chunk_size {
            ...
            if new_chunk.pattern[north_idx] == LocalTileType::Floor {
            ...
            if new_chunk.pattern[south_idx] == LocalTileType::Floor {
            ...
            if new_chunk.pattern[west_idx] == LocalTileType::Floor {
            ...
            if new_chunk.pattern[east_idx] == LocalTileType::Floor {
